...
func TestBackgroundToolWatchdogShortOverride(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		tools.Store(1)
		calls := 0
		go new(Daemon).runIdleWatchdog(ctx, 10*time.Minute, time.Minute, &last, tools.Load, &fired, &threshold, cancel, make(chan agent.Message), func() bool { calls++; cancel(); return true }, slog.Default())
		synctest.Wait()
