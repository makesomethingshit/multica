...
type AgentTaskResponse struct {
	...
	// TerminalReportGenerationFenceV1 is the claim-only capability that tells the
	// daemon this server enforces the terminal-report generation fence: the
	// dispatched_at above round-trips exactly, and /complete and /fail compare it
	// inside their terminal UPDATE. It is explicit because older servers already
	// send dispatched_at while ignoring expected_dispatched_at, so timestamp
	// presence is not evidence of the contract (MUL-7147).
