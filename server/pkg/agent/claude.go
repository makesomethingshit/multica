	...
	go func() {
		...
		for scanner.Scan() {
			...
			case "result":
				...
				resultUsage, authoritative := claudeResultUsageSince(msg, opts.Model, baseline)
				if opts.ResumeSessionID != "" && usageSnapshot == nil {
					// Resumed modelUsage is session-cumulative while usage covers this
					// main-loop turn, including through custom API endpoints. Without a
					// cost-state baseline there is nothing to subtract, so prefer the
					// turn-local usage when the SDK exposes one.
					if turnUsage, ok := claudeTurnUsage(msg, opts.Model); ok {
						resultUsage, authoritative = turnUsage, true
					}
				}
				if authoritative {
					usage = resultUsage
...
func claudeTurnUsage(msg claudeSDKMessage, fallbackModel string) (map[string]TokenUsage, bool) {
	model := msg.Model
	if model == "" {
		model = fallbackModel
	}
