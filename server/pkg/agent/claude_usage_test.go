...
func TestClaudeExecuteKeepsResumedUsageTaskLocal(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			result := executeClaudeUsageFixtureWithOptions(t, backend, 0, ExecOptions{Timeout: 5 * time.Second, ResumeSessionID: tc.resumeSessionID})
...
func TestClaudeExecuteResumedSubagentUsageGap(t *testing.T) {
	...
	backend := claudeUsageFixtureBackend(t, nil, resultEvent, false)
	result := executeClaudeUsageFixtureWithOptions(t, backend, 0, ExecOptions{Timeout: 5 * time.Second, ResumeSessionID: "session-1"})
...
func TestClaudeExecuteResumedProxyUsesTurnLocalUsage(t *testing.T) {
	...
	backend := claudeUsageFixtureBackendWithEnv(t, nil, resultEvent, false, map[string]string{"ANTHROPIC_BASE_URL": "http://127.0.0.1:4318"})
	result := executeClaudeUsageFixtureWithOptions(t, backend, 0, ExecOptions{Timeout: 5 * time.Second, ResumeSessionID: "session-1"})
