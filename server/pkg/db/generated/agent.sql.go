...
func (q *Queries) AcknowledgeExhaustedDelegatedFailureRecovery(ctx context.Context, arg AcknowledgeExhaustedDelegatedFailureRecoveryParams) (AgentTaskQueue, error) {
	...
	err := row.Scan(
		...
		&i.CancelledByName,
		&i.IssueSnapshot,
		&i.CompletionFallbackCommentID,
		&i.CompletionFallbackState,
		&i.CompletionReconcileRetryObligations,
	)
	return i, err
}
...
RETURNING task.id, task.agent_id, task.issue_id, task.status, task.priority, task.dispatched_at, task.started_at, task.completed_at, task.result, task.error, task.created_at, task.context, task.runtime_id, task.session_id, task.work_dir, task.trigger_comment_id, task.chat_session_id, task.autopilot_run_id, task.attempt, task.max_attempts, task.parent_task_id, task.failure_reason, task.trigger_summary, task.force_fresh_session, task.is_leader_task, task.wait_reason, task.initiator_user_id, task.handoff_note, task.prepare_lease_expires_at, task.squad_id, task.runtime_mcp_overlay, task.escalation_for_task_id, task.fire_at, task.originator_user_id, task.runtime_connected_apps, task.coalesced_comment_ids, task.delivered_comment_ids, task.chat_input_task_id, task.chat_finalize_deferred_at, task.originator_source, task.delegated_from_task_id, task.retry_of_task_id, task.rerun_of_task_id, task.rule_version_id, task.trigger_evidence_kind, task.trigger_evidence_ref_id, task.accountable_user_id, task.session_rollout_missing, task.retired_session_id, task.quick_actions_disabled, task.regenerate_quick_actions_for, task.branch_name, task.durable_work_dir, task.channel_context_revision, task.comment_thread_id, task.cancelled_by_type, task.cancelled_by_id, task.cancelled_by_name, task.issue_snapshot, task.completion_fallback_comment_id, task.completion_fallback_state, task.completion_reconcile_retry_obligations
`
...
const clearResolvedCompletionReconcileRetry = `-- name: ClearResolvedCompletionReconcileRetry :exec
UPDATE agent_task_queue
SET completion_reconcile_retry_obligations = (
  ...
  WHERE e->>'comment_id' IS DISTINCT FROM $1::text
)
WHERE id = $2::uuid
`

type ClearResolvedCompletionReconcileRetryParams struct {
...
func (q *Queries) ClearResolvedCompletionReconcileRetry(ctx context.Context, arg ClearResolvedCompletionReconcileRetryParams) error {
	_, err := q.db.Exec(ctx, clearResolvedCompletionReconcileRetry, arg.CommentID, arg.TaskID)
	return err
}
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, comment_thread_id, cancelled_by_type, cancelled_by_id, cancelled_by_name, issue_snapshot, completion_fallback_comment_id, completion_fallback_state, completion_reconcile_retry_obligations FROM agent_task_queue
WHERE id = $1
`
...
const hasTaskCarryingCommentForAgent = `-- name: HasTaskCarryingCommentForAgent :one
  ...
  AND (
      ...
      OR (
          ...
          AND (
              ...
              OR $4::uuid = ANY(coalesced_comment_ids)
          )
      )
      OR (status = 'completed' AND trigger_comment_id = $4::uuid)
  )
`

type HasTaskCarryingCommentForAgentParams struct {
	IssueID       pgtype.UUID `json:"issue_id"`
	AgentID       pgtype.UUID `json:"agent_id"`
...
func (q *Queries) HasTaskCarryingCommentForAgent(ctx context.Context, arg HasTaskCarryingCommentForAgentParams) (bool, error) {
	row := q.db.QueryRow(ctx, hasTaskCarryingCommentForAgent,
		arg.IssueID,
		arg.AgentID,
		arg.ExcludeTaskID,
		arg.CommentID,
...
const listCompletionReconcileRetryObligations = `-- name: ListCompletionReconcileRetryObligations :many
...
type ListCompletionReconcileRetryObligationsRow struct {
	ID                                  pgtype.UUID   `json:"id"`
	AgentID                             pgtype.UUID   `json:"agent_id"`
	IssueID                             pgtype.UUID   `json:"issue_id"`
	CompletionReconcileRetryObligations []byte        `json:"completion_reconcile_retry_obligations"`
	CoalescedCommentIds                 []pgtype.UUID `json:"coalesced_comment_ids"`
	TriggerCommentID                    pgtype.UUID   `json:"trigger_comment_id"`
}
...
// not.
func (q *Queries) ListCompletionReconcileRetryObligations(ctx context.Context, maxPerTick int32) ([]ListCompletionReconcileRetryObligationsRow, error) {
	rows, err := q.db.Query(ctx, listCompletionReconcileRetryObligations, maxPerTick)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCompletionReconcileRetryObligationsRow{}
	for rows.Next() {
		var i ListCompletionReconcileRetryObligationsRow
		if err := rows.Scan(
			&i.ID,
			&i.AgentID,
			&i.IssueID,
...
const markCompletionFallbackPending = `-- name: MarkCompletionFallbackPending :exec
UPDATE agent_task_queue
...
func (q *Queries) MarkCompletionFallbackPending(ctx context.Context, taskID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, markCompletionFallbackPending, taskID)
	return err
}
...
const recordCompletionFallbackComment = `-- name: RecordCompletionFallbackComment :exec
UPDATE agent_task_queue
SET completion_fallback_comment_id = $1::uuid,
    completion_fallback_state = 'recorded'
WHERE id = $2::uuid
`

type RecordCompletionFallbackCommentParams struct {
	FallbackID pgtype.UUID `json:"fallback_id"`
	TaskID     pgtype.UUID `json:"task_id"`
}
...
func (q *Queries) RecordCompletionFallbackComment(ctx context.Context, arg RecordCompletionFallbackCommentParams) error {
	_, err := q.db.Exec(ctx, recordCompletionFallbackComment, arg.FallbackID, arg.TaskID)
	return err
}

const recordCompletionReconcileRetryComment = `-- name: RecordCompletionReconcileRetryComment :exec
UPDATE agent_task_queue
SET completion_reconcile_retry_obligations = COALESCE(completion_reconcile_retry_obligations, '[]'::jsonb) || jsonb_build_object('comment_id', $1::uuid)
WHERE id = $2::uuid
  AND NOT EXISTS (
    SELECT 1 FROM jsonb_array_elements(COALESCE(completion_reconcile_retry_obligations, '[]'::jsonb)) AS e
    WHERE e->>'comment_id' = $1::text
  )
`

type RecordCompletionReconcileRetryCommentParams struct {
	CommentID pgtype.UUID `json:"comment_id"`
	TaskID    pgtype.UUID `json:"task_id"`
}
...
func (q *Queries) RecordCompletionReconcileRetryComment(ctx context.Context, arg RecordCompletionReconcileRetryCommentParams) error {
	_, err := q.db.Exec(ctx, recordCompletionReconcileRetryComment, arg.CommentID, arg.TaskID)
	return err
}
...
const settleCompletionFallback = `-- name: SettleCompletionFallback :exec
UPDATE agent_task_queue
SET completion_fallback_state = 'settled'
WHERE id = $1::uuid
  AND (completion_fallback_comment_id IS NULL
    OR completion_fallback_comment_id = $2::uuid)
`

type SettleCompletionFallbackParams struct {
	TaskID     pgtype.UUID `json:"task_id"`
	FallbackID pgtype.UUID `json:"fallback_id"`
}
...
func (q *Queries) SettleCompletionFallback(ctx context.Context, arg SettleCompletionFallbackParams) error {
	_, err := q.db.Exec(ctx, settleCompletionFallback, arg.TaskID, arg.FallbackID)
	return err
}
