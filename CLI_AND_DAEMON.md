...
| Workspaces root | — | `MULTICA_WORKSPACES_ROOT` | `~/multica_workspaces` for an installation that already has one, otherwise `~/multica_workspaces_<work-state key>` (see [Work state](#work-state)) |
...
- **Hermes session store reclamation** — a conversation's Hermes transcript (`state.db`) lives at `<work-state root>/hermes-sessions/<agent-id>/<hermes-profile>/<conversation>/`, outside any task directory, so a follow-up turn can resume it (see [Hermes agent memory](#hermes-agent-memory)). A store untouched for `MULTICA_GC_HERMES_SESSION_TTL` is removed. The default matches the Codex session store rather than the memory store above: these hold full transcripts, and reclaiming an idle one costs a thread that starts fresh (with a continuity notice), not an agent that forgot what it learned. A store a running task holds is never reclaimed.
- **Hermes memory store reclamation** — a Hermes agent's long-term memory (`memories/`) lives at `<work-state root>/hermes-state/<agent-id>/<hermes-profile>/`, outside any task directory, so it survives across tasks and issues (see [Hermes agent memory](#hermes-agent-memory)). A store untouched for `MULTICA_GC_HERMES_MEMORY_TTL` is removed, giving a deleted agent's memory an eventual-reclamation guarantee. The default is deliberately long: these are a handful of markdown files, and reclaiming one is user-visible amnesia rather than a cache miss. A store a running task holds is never reclaimed.
...
```
<work-state root>/hermes-state/<agent-id>/<hermes-profile>/
```
...
- **Memory is agent-scoped but runtime-local.** One agent's memory is never visible to another, and the user's own `~/.hermes/memories` is never read or written. The store lives under this machine's work-state root for the backend, so it does **not** follow the agent to another machine — an agent that runs on two runtimes has a separate memory line on each. Two profiles on one machine pointed at the same backend do share it, which is what lets that agent continue the same work under either daemon (see [Work state](#work-state)). Everything else in the home — auth, config, plugins — is still shared from the user's real home by symlink, so the agent does not need its own login.
- **To carry existing local memory in**, copy it into the store once: `cp -R ~/.hermes/memories/. "<work-state root>/hermes-state/<agent-id>/default/"`. To wipe an agent's memory, delete that directory.
- **Conversation history is covered too, in a separate store.** Hermes keeps every ACP session in `<HERMES_HOME>/state.db`, which the overlay links to a per-conversation store at `<work-state root>/hermes-sessions/<agent-id>/<hermes-profile>/<issue-id | chat_\<chat-session-id\>>/`, so a follow-up turn resumes the actual transcript. The shard is per conversation rather than per agent on purpose: tasks of one conversation run one after another, so a shard has a single writer at a time, while two issues never share a database. A host that cannot create the link (Windows without symlink privileges) keeps the database task-local instead, untouched — the link is proven creatable before anything is moved, and a copy is never used, because a copied SQLite database would absorb the turn's writes into a file the next task discards.
...
Each profile gets its own config directory (`~/.multica/profiles/<name>/`), daemon state, and health port. Daemon state means that profile's own `daemon.log`, `daemon.err.log`, and `daemon.pid` live in that directory too — see [Start](#start) for the layout, and pass `--profile <name>` to `daemon status` / `daemon logs` to act on it.

### Work state

A daemon registers with the server as one runtime per machine and backend, so its *persistent work state* follows that same boundary rather than the profile name. Two profiles pointed at the same backend — the terminal CLI's default profile and the Desktop app's `desktop-<host>` profile, typically — serve one runtime and therefore one work tree:
...
The decision is recorded once, per backend, in `~/.multica/work-state/<work-state-key>/scope.json` — the machine + backend mapping of workspaces root, provider state root and Codex namespace. That record is what makes the choice visible to every process: the Desktop app's daemon cannot see an environment variable that was exported only for your terminal, and without the record it would resolve a root of its own. Once the file exists it is authoritative. A later daemon follows it; an explicit `--workspaces-root` / `MULTICA_WORKSPACES_ROOT` that disagrees with it fails startup instead of quietly taking effect; no profile re-adopts a different legacy tree just because one appears later; and `daemon disk-usage` and other diagnostics read it without ever creating or claiming one.

Daemons in one scope also share that state's lifecycle. A task holds an OS lock on its env root and a shared lock on every provider store it mounts, so a garbage-collection pass in another daemon of the same scope — the CLI daemon and the Desktop daemon running side by side, say — skips anything in use instead of racing it, and a store being reclaimed is never mounted halfway. Two tasks of one Hermes agent still use one memory store at the same time; sharing is only narrowed against the GC, never between tasks.
...
If you need separate credentials for another organization or account — separate tokens, separate config, separate daemon lifecycle — use `--profile <name>` instead. Each profile keeps its own default workspace. Profiles do not split the machine's identity: two of them pointed at one backend are one runtime serving one work tree, and profiles pointed at different backends stay fully isolated (see [Work state](#work-state)).
