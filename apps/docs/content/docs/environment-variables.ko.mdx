...
| `MULTICA_LLM_DISABLE_THINKING` | 비어 있음(`false`) | 호환 게이트웨이 전용 thinking 비활성화 opt-in. `true`/`false`만 허용, 그 외는 시작 실패 |

`MULTICA_LLM_MAX_RETRIES`는 재시도 정책의 유일한 설정 소스입니다. 설정하지 않으면 기본값 2회, `0`이면 호출당 요청을 정확히 한 번만 보내고, 1–5면 재시도를 최대 그 횟수까지만 합니다. 할당량이 아니라 상한입니다. 재시도 대상 실패만 이를 소모하며, 성공하거나 호출자 자신의 데드라인에 걸리면 더 일찍 끝납니다. 그 밖의 값(음수, 숫자가 아닌 값, 5 초과)은 조용히 보정되지 않고 시작에 실패합니다. 상한은 지연 예산입니다. 백오프는 0.5초에서 시작해 8초 상한까지 두 배씩 늘어나므로, 더 큰 예산은 호출자 자신의 타임아웃을 넘겨 재시도 가능한 실패를 타임아웃으로 바꿉니다. 재시도는 연결 실패와 HTTP 408, 409, 429, 5xx를 대상으로 하며 그 외 4xx는 그대로 반환됩니다. 서버는 시작 시 유효한 정책을 `llm retry policy`로 기록하며 해당 줄에는 자격 증명이 포함되지 않습니다. `MULTICA_LLM_DISABLE_THINKING`은 기본적으로 비어 있음(`false`)이며, 이때 요청은 이전과 바이트 단위로 동일합니다. `true`로 설정하면 assist 레이어의 모든 Chat Completions 요청에 `{"chat_template_kwargs": {"enable_thinking": false}}`가 추가됩니다. `true`/`false`만 허용되며 그 외 값(`1`, `TRUE`, `yes` 등)은 시작에 실패합니다. 이는 호환되는 OpenAI 스타일 게이트웨이(GLM/Qwen 계열 경로 등)만 이해하는 opt-in body 확장이지 provider-neutral OpenAI API 필드가 아닙니다. 해당 필드를 거부하는 게이트웨이에서는 thinking을 다시 켜고 조용히 재시도하는 대신 요청이 실패합니다. 위 두 기능(assist 레이어)에만 적용되며 에이전트 런타임에는 전달되지 않습니다.
