...
| `MULTICA_LLM_DISABLE_THINKING` | 空（`false`） | 互換ゲートウェイ向け thinking 無効化の opt-in。`true`/`false` のみ可、それ以外は起動失敗 |

`MULTICA_LLM_MAX_RETRIES` はリトライポリシーの唯一の設定元です。未設定ならデフォルトの 2 回、`0` なら 1 回の呼び出しにつきリクエストは 1 回だけ、1〜5 ならリトライは最大その回数までになります。これは割り当てではなく上限です。消費するのはリトライ対象の失敗だけで、成功や呼び出し側自身のデッドラインによって早く終わることもあります。それ以外の値（負数、非数値、5 超過）は静かに補正されるのではなく起動に失敗します。上限はレイテンシ予算です。バックオフは 0.5 秒から始まり 8 秒を上限に倍増するため、これより大きい予算は呼び出し側自身のタイムアウトを超え、リトライ可能な失敗をタイムアウトに変えてしまいます。リトライの対象は接続失敗と HTTP 408、409、429、5xx で、それ以外の 4xx はそのまま返されます。サーバーは起動時に有効なポリシーを `llm retry policy` として記録しますが、その行に資格情報は含まれません。`MULTICA_LLM_DISABLE_THINKING` はデフォルトで空（`false`）であり、その場合リクエストは以前とバイト単位で同一です。`true` に設定すると、assist レイヤーのすべての Chat Completions リクエストに `{"chat_template_kwargs": {"enable_thinking": false}}` が追加されます。受け付けるのは `true`/`false` のみで、それ以外の値（`1`、`TRUE`、`yes` など）は起動に失敗します。これは互換性のある OpenAI 形式ゲートウェイ（GLM/Qwen 系ルートなど）のみが理解する opt-in の body 拡張であり、provider-neutral な OpenAI API フィールドではありません。当該フィールドを拒否するゲートウェイでは、thinking を再有効化して静かにリトライするのではなく、リクエストが失敗します。上記 2 機能（assist レイヤー）のみに適用され、エージェントランタイムには伝わりません。
