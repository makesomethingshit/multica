...
Cursor's verified background cleanup requires Linux 6.9 or newer (process-group pidfds), a joinable process group in the daemon's session on macOS, or a successfully assigned nested Job Object on Windows. Unsupported or unclaimable groups use the ordinary run-cancellation fallback; the daemon never signals a numeric process group after losing ownership proof.
